...
function (__java_copy_file src dest comment)
    add_custom_command(
        OUTPUT  ${dest}
        COMMAND cmake -E copy_if_different "${src}" "${dest}"
        DEPENDS ${src}
        COMMENT ${comment})
endfunction ()
...
function(add_jar _TARGET_NAME)
    ...
    if(DEFINED CMAKE_JAVA_TARGET_SOURCE_DIR)
        set(_add_jar_SOURCE_DIR "${CMAKE_JAVA_TARGET_SOURCE_DIR}")
    endif()
    ...
    cmake_parse_arguments(_add_jar
      ...
      "SOURCES;INCLUDE_JARS;SOURCE_DIR"
      ${ARGN}
    )
    ...
    if (NOT DEFINED _add_jar_SOURCE_DIR)
        set(_add_jar_SOURCE_DIR ${CMAKE_CURRENT_SOURCE_DIR})
    endif()
    ...
    foreach(_JAVA_SOURCE_FILE ${_JAVA_SOURCE_FILES})
        ...
        elseif (_JAVA_EXT STREQUAL "")
            ...
            get_filename_component(_add_jar_SOURCE_DIR_FULL ${_add_jar_SOURCE_DIR} ABSOLUTE)
            file(RELATIVE_PATH _JAVA_REL_SOURCE_PATH ${_add_jar_SOURCE_DIR_FULL} ${_JAVA_FULL})
            get_filename_component(_JAVA_REL_PATH ${_JAVA_REL_SOURCE_PATH} PATH)

            set(_JAVA_RESOURCE_FILE "${CMAKE_JAVA_CLASS_OUTPUT_PATH}/${_JAVA_REL_PATH}/${_JAVA_FILE}${_JAVA_EXT}")

            if("$ENV{VERBOSE}" STREQUAL "TRUE")
                add_custom_target( "RESOURCE_${_JAVA_FILE}"
                                ${CMAKE_COMMAND} -E copy_if_different
                                ${_JAVA_SOURCE_FILE} ${_JAVA_RESOURCE_FILE}
                                COMMENT "Copying ${_JAVA_SOURCE_FILE} to the build directory")
            else()
                add_custom_target( "RESOURCE_${_JAVA_FILE}"
                                ${CMAKE_COMMAND} -E copy_if_different
                                ${_JAVA_SOURCE_FILE} ${_JAVA_RESOURCE_FILE})
            ...
            list(APPEND _JAVA_RESOURCE_DEPS "RESOURCE_${_JAVA_FILE}")
            list(APPEND _JAVA_RESOURCE_FILES ${_JAVA_REL_PATH}/${_JAVA_FILE}${_JAVA_EXT})
        endif ()
    endforeach()
    ...
    # create java_resource_filelist
    if (NOT EXISTS ${CMAKE_JAVA_CLASS_OUTPUT_PATH}/java_resource_filelist)
        set(_RESOURCE_FILES)
        foreach(_JAVA_RESOURCE_FILE ${_JAVA_RESOURCE_FILES})
            set(_RESOURCE_FILES ${_RESOURCE_FILES}${_JAVA_RESOURCE_FILE}\n)
        endforeach()

        file(WRITE ${CMAKE_JAVA_CLASS_OUTPUT_PATH}/java_resource_filelist ${_RESOURCE_FILES})
    endif()
    ...
    if (CMAKE_JNI_TARGET)
        add_custom_command(
            ...
            COMMAND ${Java_JAR_EXECUTABLE}
                ...
                @java_resource_filelist @java_class_filelist
            COMMAND ${CMAKE_COMMAND}
            ...
            COMMAND ${CMAKE_COMMAND}
                ...
                -D_JAVA_TARGET_OUTPUT_LINK=${_JAVA_TARGET_OUTPUT_LINK}
                -P ${_JAVA_SYMLINK_SCRIPT}
            DEPENDS ${_JAVA_RESOURCE_DEPS} ${_JAVA_DEPENDS} ${CMAKE_JAVA_CLASS_OUTPUT_PATH}/java_class_filelist
